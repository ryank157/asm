...
#include <iostream>
using namespace std;
extern "C" size_t ConcatStrings_(char *des, size_t des_size,
                                 const char *const *src, size_t src_n);
void PrintResult(const char *msg, const char *des, size_t des_len,
                 const char *const *src, size_t src_n) {
  string s_test;
  const char nl = '\n';
...
int main() {
  ...
  // Empty source string
  const char *src3[] = {"Plane ", "Car ",   "",        "Truck ",
                        "Boat ",  "Train ", "Bicycle "};
